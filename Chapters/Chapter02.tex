...
familiar, making results more difficult to interpret.
...
After filtering tweet originating from our twenty cities, we were left with
\numprint{2133752}. However, there is no easy way to map twitter place id to
Foursquare venue id\footnote{It even seems that there is not one to one
correspondence between them.}. Therefore, we had to follow the short links
appearing in the tweets to recover those id. But as the dataset is rather old,
...
link to dataset on figshare or \url{http://academictorrents.com}
...
have never tweet
\href{http://twopcharts.com/twitteractivitymonitor}{\url{http://twopcharts.com/twitteractivitymonitor}}
...
% ['likes', 'users', 'checkins', 'publicness', 'density',
...
\begin{table}[p]
    ...
    \begin{tabularx}{\textwidth}{lX}
        ...
        \bottomrule
    \end{tabularx}
    \caption[Venue features]{Feature representation of Foursquare venue,
        taking surrounding into account.\label{tab:venuefeatures}}
\end{table}
...
\texttt{checkinsCount} & Total number of check-ins in Foursquare         & 2556 \\
\texttt{usersCount}    & Number of unique user in Foursquare             & 1321 \\
